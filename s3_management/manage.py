...
class S3Index:
    ...
    def to_simple_packages_html(
        ...
        print(f"DEBUG: Searching for package index.html files with prefix '{prefix_to_search}'")
        index_html_count = 0
        for obj in BUCKET.objects.filter(Prefix=prefix_to_search):
            ...
            if len(parts) == 2 and parts[1] == "index.html":
                ...
                print(f"DEBUG: Found package index.html: {obj.key} -> package '{package_name}'")
    ...
    def fetch_object_names(cls, prefix: str) -> List[str]:
        obj_names = []
        print(f"DEBUG: Starting to list objects with prefix '{prefix}' from S3", flush=True)
        print(f"DEBUG: About to call BUCKET.objects.filter(Prefix='{prefix}')...", flush=True)
        obj_count = 0
        ...
        print(f"DEBUG: Finished listing objects. Total found: {obj_count}, Accepted: {len(obj_names)}", flush=True)
        return obj_names
    ...
    def from_S3(cls, prefix: str, with_metadata: bool = True) -> "S3Index":
        ...
        print(f"DEBUG: from_S3 called with prefix='{prefix}', with_metadata={with_metadata}", flush=True)
        ...
        if prefix == "whl/nightly":
            print(f"DEBUG: Filtering nightly packages...")
            rc.objects = rc.nightly_packages_to_show()
            print(f"DEBUG: After filtering, {len(rc.objects)} nightly packages to show")
        if with_metadata:
            print(f"DEBUG: Fetching metadata for objects...")
            rc.fetch_metadata()
            print(f"DEBUG: Fetching PEP658 metadata...")
            rc.fetch_pep658()
        return rc
