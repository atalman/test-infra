...
run_smoke_tests() {
    ...
    #
    # ROCm validation runs on a CPU runner (no AMD GPU) against the regular
    # almalinux image, so the compile check falls back to the CPU/Inductor
    # path and fails with "Can't detect vectorized ISA for CPU" because that
    # image has no C++ toolchain for ISA probing. Disable it for ROCm too.
    local compile_check=""
